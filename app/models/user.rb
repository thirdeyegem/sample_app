...
class User < ActiveRecord::Base
  attr_accessor   :password #creates getter and setter methods
  attr_accessible :name, :email, :password, :password_confirmation
  
  has_many :microposts,             :dependent => :destroy #cascade delete microposts if user is deleted
  ...
  validates :password,  :presence => true,
                        :confirmation => true, #this line creates a password_confirmation attribute in Rails
                        :length => { :within => 6..40 }
  ...
  def following?(followed)
    relationships.find_by_followed_id(followed) #same as self.relationships... etc.
  end
  ...
  private
    ...
    def encrypt_password
      ...
      self.encrypted_password = encrypt(password) #set the encrypted password attribute of the object
                          #note: above refers to 'encrypt(self.password)
    end
